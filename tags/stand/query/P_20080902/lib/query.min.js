...
var Query = new (function () {
  ...
  this.read=function(key,index){key=String(key);index=Query.Type.limit(index,'number')||0;index-=(index%1);var queries=location.search.toString().replace(/^\?/,'').split(/\&/),i;var matches=[];for(i=0;i<queries.length;i+=1)if(typeof queries[i]==='string'&&queries[i].match(/[^\=]+\=?/))if(key===decodeURIComponent(queries[i].split('=')[0]))matches.push(decodeURIComponent(queries[i].split('=')[1]||''));if(index===null&&matches.length<=1)index=0;return(index===null?matches:(matches[index]));};
})();

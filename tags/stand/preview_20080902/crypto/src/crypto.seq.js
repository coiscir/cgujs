...
**/
Crypto.Sequence = function (param) {
  // enable new operator shortcut
  if (!(this instanceof Crypto.Sequence)) return new Crypto.Sequence(param);
  
  // enable cloning
  if (param instanceof Crypto.Sequence) param = param.raw();
  
  var sequence = (function () {
    if (Crypto.Type.isof(param, 'undefined', 'null')) return undefined;
    
    switch (Crypto.Type.get(param)) {
    case 'array':
      return (function (arr) {
  ...
  this.valid = function () {
    return Crypto.Type.isof(sequence, 'array');
  };
